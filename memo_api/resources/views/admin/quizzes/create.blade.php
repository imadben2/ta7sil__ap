...
@section('page-title', 'إنشاء كويز جديد')
@section('page-description', 'إضافة كويز تعليمي جديد')
...
// Load streams and subjects when year is selected
document.getElementById('year_id').addEventListener('change', async function() {
    ...
    // Clear dependent dropdowns
    streamSelect.innerHTML = '<option value="">كل الشعب (مادة مشتركة)</option>';
    subjectSelect.innerHTML = '<option value="">اختر المادة</option>';
    chapterSelect.innerHTML = '<option value="">اختر الفصل</option>';
    ...
    try {
        ...
        // Always load subjects for the year (will show all subjects for this year)
        await loadSubjects();
    } catch (error) {
...
document.getElementById('stream_id').addEventListener('change', loadSubjects);
...
async function loadSubjects() {
    ...
    // Need at least year to load subjects
    if (!yearId) return;
