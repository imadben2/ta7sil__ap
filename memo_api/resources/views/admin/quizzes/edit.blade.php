...
<div class="p-8">
    ...
    <form action="{{ route('admin.quizzes.update', $quiz->id) }}" method="POST" id="quizForm">
        @csrf
        @method('PUT')
        ...
        <div class="flex gap-6">
            ...
            <div class="flex-1">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="phase_id" id="phase_id"
                                ...
                                @foreach($phases as $phase)
                                    <option value="{{ $phase->id }}" {{ old('phase_id', $quiz->subject->phase_id ?? '') == $phase->id ? 'selected' : '' }}>
                                        {{ $phase->name_ar }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description_ar" rows="3"
                                      ...
                                      placeholder="وصف مختصر للكويز وما يغطيه">{{ old('description_ar', $quiz->description_ar) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="quiz_type" required
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <option value="practice" {{ old('quiz_type', $quiz->quiz_type) == 'practice' ? 'selected' : '' }}>تدريبي (Practice)</option>
                                <option value="timed" {{ old('quiz_type', $quiz->quiz_type) == 'timed' ? 'selected' : '' }}>موقوت (Timed)</option>
                                <option value="exam" {{ old('quiz_type', $quiz->quiz_type) == 'exam' ? 'selected' : '' }}>اختبار (Exam)</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="difficulty_level" required
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <option value="easy" {{ old('difficulty_level', $quiz->difficulty_level) == 'easy' ? 'selected' : '' }}>سهل</option>
                                <option value="medium" {{ old('difficulty_level', $quiz->difficulty_level) == 'medium' ? 'selected' : '' }}>متوسط</option>
                                <option value="hard" {{ old('difficulty_level', $quiz->difficulty_level) == 'hard' ? 'selected' : '' }}>صعب</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="estimated_duration_minutes" value="{{ old('estimated_duration_minutes', $quiz->estimated_duration_minutes) }}" min="1"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        </div>
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="allow_review" value="1" {{ old('allow_review', $quiz->allow_review) ? 'checked' : '' }}
                                   class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <div id="tags-container" class="space-y-2">
                        <div class="flex gap-2">
                            <input type="text" name="tags[]" value="{{ old('tags.0', isset($quiz->tags[0]) ? $quiz->tags[0] : '') }}"
            ...
            <div class="w-80">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="is_premium" value="1" {{ old('is_premium', $quiz->is_premium) ? 'checked' : '' }}
                                   class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
...
// Cascading dropdowns with AJAX: Phase -> Year -> Stream -> Subject -> Chapter

// Store initial values for edit mode
const initialPhaseId = '{{ old('phase_id', $quiz->subject->academicYear->academic_phase_id ?? '') }}';
const initialYearId = '{{ old('year_id', $quiz->subject->academic_year_id ?? '') }}';
const initialStreamId = '{{ old('stream_id', $quiz->subject->academic_stream_id ?? '') }}';
const initialSubjectId = '{{ old('subject_id', $quiz->subject_id) }}';
const initialChapterId = '{{ old('chapter_id', $quiz->chapter_id) }}';
...
document.getElementById('phase_id').addEventListener('change', async function() {
    ...
    try {
        ...
        years.forEach(year => {
            ...
            if (year.id == initialYearId) option.selected = true;
            yearSelect.appendChild(option);
        });

        // Trigger next level if we have initial value
        if (initialYearId && yearSelect.value) {
...
document.getElementById('year_id').addEventListener('change', async function() {
    ...
    try {
        ...
        streams.forEach(stream => {
            ...
            if (stream.id == initialStreamId) option.selected = true;
            streamSelect.appendChild(option);
        });

        // Trigger next level if we have initial value
        if (initialStreamId && streamSelect.value) {
...
document.getElementById('stream_id').addEventListener('change', async function() {
    ...
    try {
        ...
        subjects.forEach(subject => {
            ...
            if (subject.id == initialSubjectId) option.selected = true;
            subjectSelect.appendChild(option);
        });

        // Trigger next level if we have initial value
        if (initialSubjectId && subjectSelect.value) {
...
document.getElementById('subject_id').addEventListener('change', async function() {
    ...
    try {
        const response = await fetch(`/admin/quizzes/ajax/chapters/${subjectId}`);
        ...
        chapters.forEach(chapter => {
            ...
            if (chapter.id == initialChapterId) option.selected = true;
